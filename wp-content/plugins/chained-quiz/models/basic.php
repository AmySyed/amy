...
class ChainedQuiz {
   static function install($update = false) {
     ...
     if($wpdb->get_var("SHOW TABLES LIKE '".CHAINED_CHOICES."'") != CHAINED_CHOICES) {        
	  ...
	  update_option('chainedquiz_version', "0.83");
	  // exit;
   }
   ...
   static function menu() {
	...
	static function init() {
		...
		if (!session_id() and (strstr(@$_GET['page'], 'chainedquiz') or !is_admin() or (wp_doing_ajax() and !empty($_POST['chainedquiz_action']))) ) {
				@session_start();
		}
		...
		$version = get_option('chainedquiz_version');
		if(version_compare($version, '0.83') == -1) self::install(true);
	}
